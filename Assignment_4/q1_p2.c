...
void usr1_handler(){
	...
	sleep(2);
	printf("clockwise signalling in process 2\n");
	kill(next,SIGUSR1);
...
void usr2_handler(){
	...
	++count;
	sleep(2);
	printf("anti clockwise signalling in process 2\n");
	kill(prev,SIGUSR2);
}
...
int main(){
	...
	//sending p1 pid on type 1
	
	struct message_struct pid1_message;
	pid1_message.mtype = 10;
	sprintf(pid1_message.mtext,"%ld",prev);
	msgsnd(msgqid,&pid1_message,sizeof(pid1_message),0);
